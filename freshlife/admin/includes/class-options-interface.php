...
class Options_Framework_Interface {
	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			...
			if ( $value['type'] == "heading" ) {
				...
				$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="of-nav-tab ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . esc_html( $value['name'] ) . '</a></li>';
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			// Wrap all options
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "seperator" ) ) {
			...
			case "onoff":
				...
				$field_choices = array(
					array(
						'value'   => 'on',
						'label'   => __( 'On', 'freshlife' ),
						'label_class' => 'switch-label-on'
					),
					array(
						'value'   => 'off',
						'label'   => __( 'Off', 'freshlife' ),
						'label_class' => 'switch-label-off'
					)
				);

				$output .= '<div class="on-off-switch switch switch-blue">';

				foreach ( (array) $field_choices as $key => $choice ) {
					$id = $option_name . '-' . $value['id'] .'-'. $key;
					$output .= '<input class="of-input of-radio switch-input" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $choice['value'] ) . '" '. checked( $val, $choice['value'], false) .' /><label class="switch-label ' . sanitize_html_class( $choice['label_class'] ) . '" for="' . esc_attr( $id ) . '">' . esc_html( $choice['label'] ) . '</label>';
				}
