 ...
 * @return array
 */
function freshlife_entry_markup( $classes ) {
...
add_filter( 'post_class', 'freshlife_entry_markup' );
 ...
 * @return string
 */
function freshlife_comments_popup_link_attributes( $attr ) {
	return ' itemprop="discussionURL"';
}
add_filter( 'comments_popup_link_attributes', 'freshlife_comments_popup_link_attributes', 5 );
 ...
 * @return string
 */
function freshlife_get_avatar( $avatar ) {
	return preg_replace( '/(<img.*?)(\/>)/i', '$1itemprop="image" $2', $avatar );
}
add_filter( 'get_avatar', 'freshlife_get_avatar', 5 );
 ...
 * @return array
 */
function freshlife_img_attr( $attr ) {
    $attr['itemprop'] = 'image';
    return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'freshlife_img_attr', 10, 2 );