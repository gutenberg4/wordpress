 ...
 * @since      1.0.0
 */
class Freshlife_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-freshlife-popular posts-thumbnail-widget',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'freshlife' )
		);

		// Create the widget.
		parent::__construct(
			'freshlife-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts Thumbnails', 'freshlife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'freshlife_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'freshlife_popular_widget_' . $this->id, $popular );
		...
		if ( $popular->have_posts() ) {
			echo '<ul>';
				...
				while ( $popular->have_posts() ) :
					...
					echo '<li>';
						echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . get_the_post_thumbnail( get_the_ID(), 'freshlife-widget-thumb', array( 'class' => 'entry-thumb', 'alt' => esc_attr( get_the_title() ) ) ) . '</a>';
	...
	function flush_widget_transient() {
		delete_transient( 'freshlife_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'freshlife' ),
			'limit'     => 5,
			'show_date' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_date' ); ?>">
				<?php _e( 'Display post date?', 'freshlife' ); ?>
			</label>
		</p>
