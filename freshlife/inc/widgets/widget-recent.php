 ...
 * @since      1.0.0
 */
class Freshlife_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-freshlife-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'freshlife' )
		);

		// Create the widget.
		parent::__construct(
			'freshlife-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'freshlife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'freshlife_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'freshlife_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'freshlife_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'freshlife' ),
			'limit'     => 5,
			'show_date' => false
		);
