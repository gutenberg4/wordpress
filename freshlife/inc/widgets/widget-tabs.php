 ...
 * @since      1.0.0
 */
class Freshlife_Tabs_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-freshlife-tabs widget_tabs posts-thumbnail-widget',
			'description' => __( 'Display popular posts, recent posts, recent comments and tags in tabs.', 'freshlife' )
		);

		// Create the widget.
		parent::__construct(
			'freshlife-tabs',                  // $this->id_base
			__( '&raquo; Tabs', 'freshlife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		<ul class="tabs-nav">
			<li class="active"><a href="#tab1" title="<?php esc_attr_e( 'Popular', 'freshlife' ); ?>"><?php _e( 'Popular', 'freshlife' ); ?></a></li>
			<li><a href="#tab2" title="<?php esc_attr_e( 'Latest', 'freshlife' ); ?>"><?php _e( 'Latest', 'freshlife' ); ?></a></li>
			<li><a href="#tab3" title="<?php esc_attr_e( 'Comments', 'freshlife' ); ?>"><?php _e( 'Comments', 'freshlife' ); ?></a></li>        
			<li><a href="#tab4" title="<?php esc_attr_e( 'Tags', 'freshlife' ); ?>"><?php _e( 'Tags', 'freshlife' ); ?></a></li>
		</ul>

		<div class="tabs-container">
			<div class="tab-content" id="tab1">
				<?php echo freshlife_popular_posts( (int) $instance['popular_num'] ); ?>
			</div>

			<div class="tab-content" id="tab2">
				<?php echo freshlife_latest_posts( (int) $instance['recent_num'] ); ?>
			</div>
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recent_num' ); ?>">
				<?php _e( 'Number of Recent Posts', 'freshlife' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comments_num' ); ?>">
				<?php _e( 'Number of Recent Comments', 'freshlife' ); ?>
			</label>
 ...
 * @since 1.0.0
 */
function freshlife_popular_posts( $number = 5 ) {
			...
			foreach ( $popular as $post ) :
				...
				$html .= '<li class="clearfix">';
					$html .= '<a href="' . esc_url( get_permalink( $post->ID ) ) . '" rel="bookmark">';
						$html .= get_the_post_thumbnail( $post->ID, 'freshlife-widget-thumb', array( 'class' => 'entry-thumbnail', 'alt' => esc_attr( get_the_title( $post->ID ) ) ) );
 ...
 * @since 1.0.0
 */
function freshlife_latest_posts( $number = 5 ) {
