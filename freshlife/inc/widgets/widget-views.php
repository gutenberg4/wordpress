 ...
 * @since      1.0.0
 */
class Freshlife_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-freshlife-views posts-thumbnail-widget',
			'description' => __( 'Display the most views posts with thumbnails.', 'freshlife' )
		);

		// Create the widget.
		parent::__construct(
			'freshlife-views',                                        // $this->id_base
			__( '&raquo; Most Views Posts Thumbnails', 'freshlife' ), // $this->name
		...
		if ( $views->have_posts() ) {
			echo '<ul>';
				...
				while ( $views->have_posts() ) :
					...
					// Get the views.
					$view = intval( get_post_meta( $post->ID, 'Views', true ) );
					...
					echo '<li>';
						...
						echo '<div class="entry-info">';
							...
							if ( $instance['show_view'] ) :
								echo '<span class="entry-view">- ' .  number_format_i18n( $view ) . __( ' Views', 'freshlife' ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'freshlife' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
